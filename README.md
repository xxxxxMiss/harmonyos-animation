...
**复杂测试案例：** 页面顶部有一条渲染延迟档位条（即时 / 手机 120ms / 手表 400-600ms /
**手表 400-900ms · 分 3 段**）。最后那档模拟手表上「骨架 → 半高 → 定高」的分段落位，
一页能产生上百次布局变化 —— 这正是锚定最容易失效的场景，也是本轮修掉两个真 bug 的起因。
所有时间只在 `model/RenderLatency.ets` 里，`scroll/` 下仍然零墙钟等待。

不需要设备就能复现这类失效：

```bash
node tools/simulate-anchor.mjs        # 期望 PASS，finalErr 全为 0vp
```
